        ...
        QObject::connect(window, &QQuickWindow::visibleChanged, [&, window]() {
            ...
            if (direction_sign > 0) {
                for (int i = _notify_windows_list.size() - 1; i >= 0; i--) {
                    qreal wPos = _notify_windows_list[i]->property("y").toReal();

                    if (_notify_windows_list[i] != window && wPos > closedWindowPos && i > index) {
                        _notify_windows_list[i]->setProperty(
                         "y", QVariant::fromValue(wPos - window->height() - margin));
                    }
                }
            } else {
                for (int i = 0; i < _notify_windows_list.size(); i++) {
                    ...
                    if (_notify_windows_list[i] != window && wPos < closedWindowPos) {
                        _notify_windows_list[i]->setProperty(
                         "y", QVariant::fromValue(wPos + window->height() + margin));
                    }
                }
